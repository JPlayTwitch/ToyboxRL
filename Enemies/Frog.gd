	...
	if self.visible:
		...
		if path:
			...
			if path.size()<4:
				...
				if hit:
					mess_log.append_bbcode("\n [color=#137f07]Toy Frog[/color] tongued at you for [color=#ff0000]" + str(dmg) + "[/color] damage")
					game.damage_player(dmg)
				else:
					mess_log.append_bbcode("\n [color=#137f07]Toy Frog[/color] swiped the air fruitlessly")
				...
				var move_tile = Vector2(path[2].x, path[2].y)
				var blocked = false
				for enemy in game.enemies:
					if enemy.tile == move_tile:
						...
						break
				if blocked:
					move_tile = Vector2(path[1].x, path[1].y)
					blocked = false
					for enemy in game.enemies:
						if enemy.tile == move_tile:
							blocked = true
...
func take_damage(game,dmg):
	...
	if hit:
		hp = max(0, hp-dmg)
		mess_log.append_bbcode("\n You hit [color=#137f07]Toy Frog[/color] for [color=#00ff00]" + str(dmg) + "[/color] damage")
		
		if hp == 0:
			mess_log.append_bbcode("\n [color=#137f07]Toy Frog[/color] died")
		if hp == 0:
			dead = true#
	else:
		mess_log.append_bbcode("\n You missed the [color=#137f07]Toy Frog[/color]")
